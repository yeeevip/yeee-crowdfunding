  ...
  <div class="mod-qrtzLog">
    <el-form :inline="true" :model="dataForm" @keyup.enter="listData()">
      ...
      <el-form-item>
        ...
        <el-button v-if="$hasPerm('sys:qrtzLog:add')" type="primary" @click="editHandle()">新增</el-button>
        <el-button v-if="$hasPerm('sys:qrtzLog:del')" type="danger" @click="delHandle()" :disabled="dataListSelections.length <= 0">删除</el-button>
        <el-button v-if="$hasPerm('sys:qrtzLog:expt')" @click="exptHandle()">导出</el-button>
        <el-button v-if="$hasPerm('sys:qrtzLog:impt')" @click="imptHandle()">导入</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border stripe v-loading="dataListLoading" :max-height="tableHeight" 
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="100">
        <template #default="scope">
          <el-button v-if="$hasPerm('sys:qrtzLog:info')" type="primary" link @click="infoHandle(scope.row.id)" title="详情">
            <el-icon><Document /></el-icon>
          </el-button>
          <el-button v-if="$hasPerm('sys:qrtzLog:del')" type="danger" link @click="delHandle(scope.row.id)" title="删除">
...
<script>
  ...
  import info from './qrtzLog-info.vue'
