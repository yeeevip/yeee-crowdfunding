    ...
    methods: {
      ...
      listOrg () {
        this.$http.get('/manage/sys-dept/list-set', {
          ...
          this.$nextTick(() => {
            // 确保树形组件引用存在
            const orgTreeRef = this.$refs.orgListTree
            if (orgTreeRef && res.data.checkedKeys) {
              res.data.checkedKeys.forEach(item => orgTreeRef.setChecked(item, true))
            }
          })
        }).catch(() => {})
      },
      ...
      dataFormSubmit: debounce(function () {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            // 确保树形组件引用存在
            const orgTreeRef = this.$refs.orgListTree
            this.dataForm.orgIds = orgTreeRef ? [
              ...orgTreeRef.getCheckedKeys(),
              ...orgTreeRef.getHalfCheckedKeys()
            ] : []
