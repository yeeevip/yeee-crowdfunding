  ...
  <div class="mod-classify">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="listData()">
      ...
      <el-form-item>
        ...
        <el-button v-if="$hasPerm('blog:classify:add')" type="primary" size="default" @click="editHandle()">新增</el-button>
        <el-button v-if="$hasPerm('blog:classify:del')" type="danger" size="default" @click="delHandle()" :disabled="dataListSelections.length <= 0">删除</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border stripe v-loading="dataListLoading" :max-height="tableHeight"
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="150">
        <template #default="scope">
          <el-button v-if="$hasPerm('blog:classify:info')" type="primary" link size="small" @click="infoHandle(scope.row.id)" :icon="Document" title="详情"></el-button>
          <el-button v-if="$hasPerm('blog:classify:upd')" type="primary" link size="small" @click="editHandle(scope.row.id)" :icon="Edit" title="编辑"></el-button>
          <el-button v-if="$hasPerm('blog:classify:del')" type="danger" link size="small" @click="delHandle(scope.row.id)" :icon="Delete" title="删除"></el-button>
        </template>
      </el-table-column>
  </el-table>
...
import info from './classify-info.vue'
