  ...
  <div class="mod-label">
    <el-form :inline="true" :model="dataForm" @keyup.enter.native="listData()">
      ...
      <el-form-item>
        ...
        <el-button v-if="$hasPerm('blog:label:add')" type="primary" size="default" @click="editHandle()">新增</el-button>
        <el-button v-if="$hasPerm('blog:label:del')" type="danger" size="default" @click="delHandle()" :disabled="dataListSelections.length <= 0">删除</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="dataList" border stripe v-loading="dataListLoading" :max-height="tableHeight"
      ...
      <el-table-column prop="type" label="类型" sortable="custom" header-align="center" align="center">
        <template #default="scope">
          ...
          <el-tag v-else-if="scope.row.type === 2" type="danger">业务</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" fixed="right" header-align="center" align="center" width="150">
        <template #default="scope">
          <el-button v-if="$hasPerm('blog:label:info')" type="primary" link size="small" @click="infoHandle(scope.row.id)" :icon="Document" title="详情"></el-button>
          <el-button v-if="$hasPerm('blog:label:upd')" type="primary" link size="small" @click="editHandle(scope.row.id)" :icon="Edit" title="编辑"></el-button>
          <el-button v-if="$hasPerm('blog:label:del')" type="danger" link size="small" @click="delHandle(scope.row.id)" :icon="Delete" title="删除"></el-button>
        </template>
      </el-table-column>
  </el-table>
...
<script>
import edit from './label-edit.vue'
