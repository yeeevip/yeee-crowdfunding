...
$(document).ready(function(){
	...
	$("#adminLoginBtn").click(function () {
		$.ajax({
			type: 'POST',
			async: false,
			url: API_BASE_URL + '/manage/sys-user/login' ,
			...
			success: function (res) {
				if (res.code != 200) {
					...
					// 将token存储到本地
					localStorage.setItem('yeee-manageweb-token', JSON.stringify(res.data))
					// 请求成功后跳转到首页
	...
	$("#user_menu").click(function(){
		...
		$.ajax({	
			url: API_BASE_URL + "/manage/cf/user/page",
			async: false,   //是否为异步请求
			type: "POST", //请求方式为POST);
	...
	$("#dingdan_menu").click(function(){
		let token = localStorage.getItem("yeee-manageweb-token");
		$.ajax({
			url: API_BASE_URL + "/manage/cf/order/page",
			async: false,   //是否为异步请求
			type: "POST", //请求方式为POST);
			contentType: "application/json;charset=utf-8",
			data: JSON.stringify({
	...
	$("#category_menu").click(function(){
		let token = localStorage.getItem("yeee-manageweb-token");
		$.ajax({
			url: API_BASE_URL + "/manage/cf/category/page",
			async: false,   //是否为异步请求
			type: "POST", //请求方式为POST);
			contentType: "application/json;charset=utf-8",
			data: JSON.stringify({
...
//			alert($("#u_allCheck").prop("checked"));
	...
	$("#project_menu").click(function(){
		let token = localStorage.getItem("yeee-manageweb-token");
		$.ajax({
			url: API_BASE_URL + "/manage/cf/project/page",
			async: false,   //是否为异步请求
			type: "POST", //请求方式为POST);
			contentType: "application/json;charset=utf-8",
			data: JSON.stringify({
...
function systemUser(){
	...
	let token = localStorage.getItem("yeee-manageweb-token");
	$.ajax({
		url: API_BASE_URL + "/manage/sys-user/page",
		async: false,   //是否为异步请求
		type: "POST", //请求方式为POST);
		contentType: "application/json;charset=utf-8",
		data: JSON.stringify({
